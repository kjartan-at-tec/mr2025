...
\begin{frame}[label={sec:org900d457}]{System identification}
\small
...
\begin{frame}[label={sec:org224d5a4}]{The shift operator \(\shift\)}
...
\begin{frame}[label={sec:orgd667275}]{The problem with least squares}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{align*}
...
\begin{frame}[label={sec:orgcca3410}]{More robust: The Huber loss function}
\begin{columns}
\begin{column}{0.4\columnwidth}
...
\begin{frame}[label={sec:org0234446}]{First-order ARX model without delay}
\begin{center}
...
\begin{frame}[label={sec:org7db8caa}]{The ARX model}
\[ A(\shift) y(k) = B(\shift)u(k) + e(k+n)\]
